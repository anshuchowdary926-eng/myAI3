...
function isSchengenQuery(text: string): boolean {
  ...
  if (visaKeywords.some((w) => m.includes(w))) return true;
...
const CAPABILITY_RESPONSE =
  ...
  "Ask me anything related to Schengen visas, and I’ll guide you step by step.";
...
export default function Chat() {
  ...
  const { messages, sendMessage, status, stop, setMessages } = useChat({
    ...
    async onSend(message) {
      ...
      // 1. "What can you do" → fixed capability reply
      ...
      // 2. If it's clearly not a Schengen visa query → block
      if (!isSchengenQuery(m)) {
        return {
          role: "assistant",
          content:
            "Sorry, I’m only built to answer Schengen visa questions. I can’t help with this kind of request.",
        };
      }

      // 3. Otherwise, let the normal backend / model handle it
      return null;
    },
  });
